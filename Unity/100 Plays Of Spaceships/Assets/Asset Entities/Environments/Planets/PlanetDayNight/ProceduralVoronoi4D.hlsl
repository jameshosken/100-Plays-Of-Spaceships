...
//uniform float _Frequency, _Lacunarity, _Gain, _Jitter, _Amp;
void voronoi4D_float(in float freq,
	in float lac, in float gain,
	in float jit, in float amp, in int octaves,
	in float3 pos, in float evolution, out float value) {
	_Frequency = freq;
	_Lacunarity = lac; 
	_Gain = gain;
	_Jitter = jit;
	...
	float4 p = float4(pos.x, pos.y, pos.z, evolution);
	value = fBm_F0(p, octaves);
	return;
