...
public class MarchingCubeGenerator : MonoBehaviour
{
    ...
    enum WorldType {Noise, Sphere, SphereNoise}
    ...
    {
        if(type == WorldType.Noise)
        {
            GenerateNoiseWorld(sizeX, sizeY, sizeZ);
        }
        if (type == WorldType.Sphere)
        {
    ...
    void GenerateNoiseWorld(int sizeX, int sizeY, int sizeZ)
    {
        ...
        for (int x = 0; x < sizeX; x++)
        {
            for (int y = 0; y < sizeY; y++)
            {
                for (int z = 0; z < sizeZ; z++)
                {
                    cubeWorld[x, y, z] = Map(cubeWorld[x, y, z], minVal, maxVal, 0f, 1f);
                }
            }
        }
    }
        ...
        for (int x = 0; x < sizeX; x++)
        {
            for (int y = 0; y < sizeY; y++)
            {
                for (int z = 0; z < sizeZ; z++)
                {
                    ...
                    cubeWorld[x, y, z] = Map(cubeWorld[x, y, z], minVal, maxVal, 0f, 10f) * distNormalised;
                }
            }
        }
    }
