...
void noise4D_float(in float3 pos, in float evolution, out float output) {
	float4 vec = float4(pos.x, pos.y, pos.z, evolution);
	output = snoise(vec);
	return;
}


void noise4D_octaves_float(in float3 pos, in float evolution, in int octaves, in float lacunarity, in float persistence,
	out float output) {
	...
	{
		float sampleX = (pos.x) / 1 * freq;   //Heigher the freq, the further apart the sample points
		float sampleY = (pos.y) / 1 * freq;
		float sampleZ = (pos.z) / 1 * freq;

		float4 vec = float4(sampleX * (i + 1), sampleY * (i + 1), sampleZ * (i + 1), evolution);
		float perlinValue = snoise(vec);

		cumulative += perlinValue * amplitude;
